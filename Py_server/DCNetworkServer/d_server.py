...
class DiningServer:
    def __init__(self):
        ...
        self.key_words = cfg['self_parameters']['about_number_message']
        self.second_key_words = cfg['self_parameters']['broadcast_desire_message']
        self.map_of_guests = {}
        self.map_of_descriptors = {}
        self.order_messages_list = []
    ...
    async def read_data_from_socket(self, reader, writer):
        ...
        if message == self.key_words:
            if current_address not in self.map_of_guests:
                ...
                self.map_of_descriptors[self.current_number] = current_address
        ...
        elif message == self.second_key_words:
            if current_address not in self.map_of_guests:
                self.map_of_guests[current_address] = self.current_number
                self.map_of_descriptors[self.current_number] = current_address
                ...
                self.order_messages_list.append(self.current_number)
                self.current_number += 1
                print(self.map_of_guests)
